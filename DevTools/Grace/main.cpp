...
void load_grace_json_files(Vector<String> files)
{
...
int main(int argc, char** argv)
{
    ...
    if (settings.get("root", &root)) {
        fprintf(stderr, "Searching for grace json files in: %s\n", root.characters());
        files = FileProvider::the().glob_all_grace_json_files(root);
        for (auto& file : files) {
            fprintf(stderr, "* %s\n", file.characters());
        }
    } else {
